...
procedure can_ring_info(alpha, points)
  "---------------------------------------------------------------------------";
  // "";
  ...
  "Degree:", s;
  if (s le 0) then
    return; // the canonical ring is trivial
  end if;
  n := #alpha;
  ...
  while true do
    ...
    if (#gen_idxs lt r+1) then
      for q in [0..r] do
        ...
        gen := V![Coefficient((t - points[1])^q,j)
        : j in [0..r]];
        if (gen in W) then
          if factor then 
...
procedure can_ring_one_moving_pt(alpha)
  // Choose a finite field of sufficiently high characteristic for speed.
  k := /*Rationals();*/ GF(101);
  F<a> := FunctionField(k);

  can_ring_info(alpha, [a] cat [0..#alpha-2]);
end procedure;

procedure can_ring_all_moving_pts(alpha)
  K<[a]> := FunctionField(Rationals(), #alpha);
  can_ring_info(alpha, [a[i] : i in [1..#alpha]]);
end procedure;
